...
class ProjectEditor:
    ...
    def __init__(self, root: tk.Tk, bg_color: str):
        ...
        tk.Checkbutton(
            ...
            command=self._toggle_url_state,
        ...
        tk.Label(techstack_frame, text="Tech Stack (seperate by semicolon;)").pack(
            side="left"
        )
        techstack_gen_btn = _Button(
            techstack_frame,
            "Generate from GitHub",
            command=lambda: self._get_techstack_from_github(),
        )
        ...
        self.excerpt.add_buttons(
            _Button(
                ...
                command=lambda: self._generate_excerpt_from_description(),
            )
        )
        ...
        self.description.add_buttons(
            _Button(
                None,
                "AI-Generate from README.md",
                command=lambda: self._generate_description_from_readme(),
            )
        )
    ...
    def _get_techstack_from_github(self):
        if self.techstack.get() != "":
            confirm = messagebox.askyesno(
                "Confirm", "This will overwrite the current tech-stack, are you sure?"
            )
            if not confirm:
                return
        ...
        self.techstack.insert(
            0, lang_text
        )  # TODO actually save this and add it to Content class

    def _generate_excerpt_from_description(self):
        if self.excerpt.get_text() != "":
            confirm = messagebox.askyesno(
                "Confirm", "This will overwrite the current excerpt, are you sure?"
            )
            if not confirm:
                return
        ...
        if self.description.get_text() == "":
            confirm = messagebox.showinfo(
                ...
                "Sorry! We can't generate an excerpt without a description...",
            )
            return

        ai_excerpt = ChatGPT.create_excerpt_from_description(
    ...
    def _generate_description_from_readme(self):
        if self.description.get_text() != "":
            confirm = messagebox.askyesno(
                "Confirm", "This will overwrite the current description, are you sure?"
            )
            if not confirm:
                return
